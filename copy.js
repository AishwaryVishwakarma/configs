...
const submodulePath = path.join(__dirname, '..', 'temp');
const destinationPath = path.join(__dirname, '..');

// List of files/folders to exclude
const excludeFiles = ['.git', 'copy.js'];
...
copyFiles(submodulePath, destinationPath)
  .then(({ copiedFiles, skippedFiles }) => {
    console.log(`${GREEN}Copied ${copiedFiles} files.${RESET} ${CYAN}Skipped ${skippedFiles} files.${RESET}`);
